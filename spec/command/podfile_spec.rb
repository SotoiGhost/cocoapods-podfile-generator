    ...
    describe 'CLAide' do
      ...
      describe "Validate the command" do
        ...
        describe "Validate text files" do
          ...
          #   lambda { Command.parse(%W{ podfile --#{CocoapodsPodfileGenerator::TEXT_FILE_OPTION_NAME}=./spec/Non_existing_file.txt }).validate! }
          #     .should.raise(CLAide::Help)
          #     .message.should.match(/The file was not found/)
          # end

          # it "parses a text file correctly" do
          #   lambda { Command.parse(%W{ podfile --#{CocoapodsPodfileGenerator::TEXT_FILE_OPTION_NAME}=./spec/Pods.txt }).validate! }
          #     .should.not.raise()
          # end
    
          # it "fails when there's a bad line to parse in a text file" do
          #   lambda { Command.parse(%W{ podfile --#{CocoapodsPodfileGenerator::TEXT_FILE_OPTION_NAME}=./spec/Bad_Pods_Format.txt }).validate! }
          #     .should.raise(CLAide::Help)
          #     .message.should.match(/There was a problem parsing/)
          # end

          # it "fails when a text file does not have the .txt extension" do
          #   lambda { Command.parse(%W{ podfile --#{CocoapodsPodfileGenerator::TEXT_FILE_OPTION_NAME}=./spec/TextFile }).validate! }
          #     .should.raise(CLAide::Help)
          #     .message.should.match(/should have a .txt extension./)
          # end
          
          # it "fails when a Pod in a text file does not exist" do
          #   lambda { Command.parse(%W{ podfile --#{CocoapodsPodfileGenerator::TEXT_FILE_OPTION_NAME}=./spec/Non_Existing_Pod.txt }).validate! }
          #     .should.raise(PodfileGeneratorInformative)
          #     .message.should.match(/There was a problem/)
          # end
        end

        describe "Validate json files" do
          it "fails when the json file given does not exist" do
            lambda { Command.parse(%W{ podfile --#{CocoapodsPodfileGenerator::JSON_FILE_OPTION_NAME}=./spec/Non_existing_file.json }).validate! }
              .should.raise(CLAide::Help)
              .message.should.match(/The file was not found/)
          end
          
          it "parses a json file correctly" do
            lambda { Command.parse(%W{ podfile --#{CocoapodsPodfileGenerator::JSON_FILE_OPTION_NAME}=./spec/Pods.json }).validate! }
              .should.not.raise
          end

          it "fails when the json file has a bad format" do
            lambda { Command.parse(%W{ podfile --#{CocoapodsPodfileGenerator::JSON_FILE_OPTION_NAME}=./spec/Bad_Json_Format.json }).validate! }
              .should.raise(PodfileGeneratorInformative)
              .message.should.match(/There was a problem parsing/)
          end

          it "fails when a json file does not have the .json extension" do
            lambda { Command.parse(%W{ podfile --#{CocoapodsPodfileGenerator::JSON_FILE_OPTION_NAME}=./spec/JsonFile }).validate! }
              .should.raise(CLAide::Help)
              .message.should.match(/should have a .json extension./)
          end

          it "does not validate a Pod in a json file when does not have a version" do
            lambda { Command.parse(%W{ podfile --#{CocoapodsPodfileGenerator::JSON_FILE_OPTION_NAME}=./spec/Pods_with_data.json }).validate! }
              .should.not.raise
          end
        end
      end
      ...
      describe "Test using only Pods as args" do
        ...
        #   podfile = Command.parse(%W{ podfile --#{CocoapodsPodfileGenerator::TEXT_FILE_OPTION_NAME}=./spec/Pods.txt })
        #   podfile.validate!
        #   podfile.run
        #   @default_podfile_pathname.exist?.should.be.true?
        #   @default_podfile_pathname.empty?.should.be.false?
        ...
        #   podfile = Command.parse(%W{ podfile --#{CocoapodsPodfileGenerator::TEXT_FILE_OPTION_NAME}=./spec/Pods.txt --#{CocoapodsPodfileGenerator::OUTPUT_OPTION_NAME}=#{@podfile_pathname} })
        #   podfile.validate!
        #   podfile.run
        #   @podfile_pathname.exist?.should.be.true?
        #   @podfile_pathname.empty?.should.be.false?
        ...
        #     "podfile", 
        #     *pod_args, 
        #     "--#{CocoapodsPodfileGenerator::TEXT_FILE_OPTION_NAME}=./spec/Pods.txt"
        #   ])
        #   podfile.validate!
        #   podfile.run
        #   @default_podfile_pathname.exist?.should.be.true?
        ...
        it "Json Test" do
          ...
          podfile = Command.parse([
            "podfile",
            # *pod_args,
            "--#{CocoapodsPodfileGenerator::JSON_FILE_OPTION_NAME}=./spec/Pods.json",
            "--#{CocoapodsPodfileGenerator::OUTPUT_OPTION_NAME}=#{@podfile_pathname}"])
          podfile.validate!
          puts podfile.run
          true.should.be.true?
